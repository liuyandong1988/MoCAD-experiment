"""
MoCAD experiment 4 : Collecting data
UE4 editor
carla 0.9.9.4
-----------------------------------------------------------
1. move the vehicle
2. states -- action: image -- steer
3. modify pythonAPI manual_control.py
...
class KeyboardControl(object):
    ...
    def _parse_vehicle_keys(self, keys, milliseconds):
        ...
        # change the steer increment to improve the sensitivity
...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image):
        ...
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            # change the image to pygame show format
        ...
        # save the image
        if self.recording:
            self.image_name = round(image.frame, 8)
            image.save_to_disk('asyc_man_image/%d' % self.image_name)
...
def game_loop(args):
    ...
    try:
        ...
        # show the information in the pygame screen
        ...
        # --- record the image--action pair --- #
        with open('asyc_man_driving_log.csv', 'a') as csv_file:
            ...
            while True:
                ...
                if controller.parse_events(client, world, clock):
                    return
                # read the server frame
                world.tick(clock)
                # show the rgb-image
                world.render(display)
                # update the whole surface image
                pygame.display.flip()
...
def main():
    ...
    argparser.add_argument(
        ...
        # default='1280x720',
        default='256x128',  # save the image size
